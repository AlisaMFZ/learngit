...
function checkNoRef(schema) {
  ...
  if (Array.isArray(schema)) {
    for (var i = 0; i < schema.length; i++) {
      item = schema[i];
      if ((typeof item === 'undefined' ? 'undefined' : (0, _typeof3.default)(item)) == 'object' && !checkNoRef(item)) return false;
    }
  } else {
...
function countKeys(schema) {
  ...
  if (Array.isArray(schema)) {
    for (var i = 0; i < schema.length; i++) {
      item = schema[i];
      if ((typeof item === 'undefined' ? 'undefined' : (0, _typeof3.default)(item)) == 'object') count += countKeys(item);
    ...
    for (var key in schema) {
      ...
      if (SIMPLE_INLINED[key]) {
        ...
        item = schema[key];
        if ((typeof item === 'undefined' ? 'undefined' : (0, _typeof3.default)(item)) == 'object') count += countKeys(item) + 1;
        if (count == Infinity) return Infinity;
      }
    }
  }
