...
function merge(from, to) {
  assert.ok(from);
  assert.equal(typeof from === 'undefined' ? 'undefined' : (0, _typeof3.default)(from), 'object');
  assert.ok(to);
  assert.equal(typeof to === 'undefined' ? 'undefined' : (0, _typeof3.default)(to), 'object');

  var keys = (0, _getOwnPropertyNames2.default)(from);
  keys.forEach(function (key) {
    if (to[key]) return;

    var value = (0, _getOwnPropertyDescriptor2.default)(from, key);
    (0, _defineProperty2.default)(to, key, value);
  });
...
Writer.prototype.writeString = function (s, tag) {
  if (typeof s !== 'string') throw new TypeError('argument must be a string (was: ' + (typeof s === 'undefined' ? 'undefined' : (0, _typeof3.default)(s)) + ')');
