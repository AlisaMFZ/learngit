...
dumpScript = function dumpScript(name) {
  var defer, fs, path;
  fs = require('fs');
  path = require('path');
  defer = Q.defer();
  fs.readFile(path.resolve(__dirname, 'completion.sh'), 'utf8', function (err, d) {
    ...
    _onError = function onError(err) {
      if (err.errno === require('constants').EPIPE) {
        process.stdout.removeListener('error', _onError);
        return defer.resolve();
      } else {
        return defer.reject(err);
      }
    };
    process.stdout.on('error', _onError);
...
complete = function complete(cmd, opts) {
  var compls, m, o, opt, optPrefix, optWord;
  compls = [];
  if (opts.partialWord.indexOf('-')) {
    compls = (0, _keys2.default)(cmd._cmdsByName);
  } else {
    if (m = opts.partialWord.match(/^(--\w[\w-_]*)=(.*)$/)) {
      optWord = m[1];
      optPrefix = optWord + '=';
    } else {
      compls = (0, _keys2.default)(cmd._optsByKey);
    }
  }
