...
module.exports = function (dir) {
  ...
  // ignore index file
  .filter(function (f) {
    return f.match(/^index\./) ? false : true;
  })
  // ignore non-js files that aren't folders
  .filter(function (f) {
    return isDir(dir, f) || f.match(extensions_regex()) ? true : false;
  })
  // ignore folders without an index file
  .filter(function (f) {
    return isDir(dir, f) && !contains_index(dir, f) ? false : true;
  })
  // remove extensions
  .map(function (f) {
    return f.replace(extensions_regex(), '');
  });
...
function contains_index(dir, f) {
  var res = false;
  valid_extensions.forEach(function (ext) {
    if (fs.existsSync(path.join(dir, f, 'index.' + ext))) {
      res = true;
    }
  });
  return res;
