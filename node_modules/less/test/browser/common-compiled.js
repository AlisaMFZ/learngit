...
var testLessEqualsInDocument = function testLessEqualsInDocument() {
    testLessInDocument(testSheet);
};

var testLessErrorsInDocument = function testLessErrorsInDocument(isConsole) {
    testLessInDocument(isConsole ? testErrorSheetConsole : testErrorSheet);
};

var testLessInDocument = function testLessInDocument(testFunc) {
...
var ieFormat = function ieFormat(text) {
...
var testSheet = function testSheet(sheet) {
    it(sheet.id + " should match the expected output", function (done) {
        ...
        less.pageLoadFinished.then(function () {
            ...
            lessOutput = lessOutputObj.styleSheet ? lessOutputObj.styleSheet.cssText : lessOutputObj.innerText || lessOutputObj.innerHTML;
...
var waitFor = function waitFor(waitFunc) {
...
var testErrorSheet = function testErrorSheet(sheet) {
    it(sheet.id + " should match an error", function (done) {
        ...
        waitFor(function () {
            ...
            var innerText = actualErrorElement.innerHTML.replace(/<h3>|<\/?p>|<a href="[^"]*">|<\/a>|<ul>|<\/?pre( class="?[^">]*"?)?>|<\/li>|<\/?label>/ig, "").replace(/<\/h3>/ig, " ").replace(/<li>|<\/ul>|<br>/ig, "\n").replace(/&amp;/ig, "&")
            // for IE8
            .replace(/\r\n/g, "\n").replace(/\. \nin/, ". in");
            actualErrorMsg = innerText.replace(/\n\d+/g, function (lineNo) {
                return lineNo + " ";
            }).replace(/\n\s*in /g, " in ").replace(/\n{2,}/g, "\n").replace(/\nStack Trace\n[\s\S]*/i, "").replace(/\n$/, "");
            errorFile.then(function (errorTxt) {
                errorTxt = errorTxt.replace(/\{path\}/g, "").replace(/\{pathrel\}/g, "").replace(/\{pathhref\}/g, "http://localhost:8081/test/less/errors/").replace(/\{404status\}/g, " (404)").replace(/\{node\}.*\{\/node\}/g, "").replace(/\n$/, "");
...
var testErrorSheetConsole = function testErrorSheetConsole(sheet) {
    it(sheet.id + " should match an error", function (done) {
        var lessHref = sheet.href,
        ...
        errorFile.then(function (errorTxt) {
            errorTxt.replace(/\{path\}/g, "").replace(/\{pathrel\}/g, "").replace(/\{pathhref\}/g, "http://localhost:8081/browser/less/").replace(/\{404status\}/g, " (404)").replace(/\{node\}.*\{\/node\}/g, "").trim();
            expect(actualErrorMsg).toEqual(errorTxt);
            done();
        });
    });
...
var loadFile = function loadFile(href) {
