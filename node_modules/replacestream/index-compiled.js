...
module.exports = function ReplaceStream(search, replace, options) {
  ...
  function transform(buf, enc, cb) {
    ...
    if (tail.length < 1) tail = haystack.slice(lastPos) > options.maxMatchLen ? haystack.slice(lastPos).slice(0 - options.maxMatchLen) : haystack.slice(lastPos);
...
function createReplaceFn(replace, isRegEx) {
  var regexReplaceFunction = function regexReplaceFunction() {
