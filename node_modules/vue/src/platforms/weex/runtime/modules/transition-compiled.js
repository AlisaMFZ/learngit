...
function enter(_, vnode) {
  ...
  var needAnimation = (0, _keys2.default)(endState).length > 0;
  ...
  if (startState) {
    for (var key in startState) {
      ...
      if (process.env.NODE_ENV !== 'production' && targetState[key] == null && (!activeState || activeState[key] == null) && (!endState || endState[key] == null)) {
        (0, _debug.warn)('transition property "' + key + '" is declared in enter starting class (.' + startClass + '), ' + ('but not declared anywhere in enter ending class (.' + endClass + '), ') + ('enter active cass (.' + activeClass + ') or the element\'s default styling. ') + 'Note in Weex, CSS properties need explicit values to be transitionable.');
      }
    }
  }
