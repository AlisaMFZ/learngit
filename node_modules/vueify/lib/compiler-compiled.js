...
var splitRE = /\r?\n/g;
var resolvedPartsCache = (0, _create2.default)(null);
...
compiler.applyConfig = function (config) {
  ...
  (0, _keys2.default)(config).forEach(function (key) {
    if (key !== 'customCompilers') {
      ...
      // register compilers
      (0, _keys2.default)(config[key]).forEach(function (name) {
...
compiler.compile = function (content, filePath, cb) {
  ...
  _promise2.default.all([processTemplate(parts.template, filePath, resolvedParts), processScript(parts.script, filePath, resolvedParts)].concat(parts.styles.map(function (style) {
    return processStyle(style, filePath, id, resolvedParts);
  }))).then(mergeParts).catch(cb);
  ...
  function mergeParts() {
    ...
    if (style && !isServer) {
      ...
      if (!options.extractCSS) {
        style = (0, _stringify2.default)(style);
    ...
    output += 'var __vue__options__ = (typeof module.exports === "function"' + '? module.exports.options' + ': module.exports)\n';
    // template
    var template = resolvedParts.template;
    if (template) {
      if (!isProduction && !isServer) {
        output += 'if (__vue__options__.functional) {console.error("' + '[vueify] functional components are not supported and ' + 'should be defined in plain js files using render functions.' + '")}\n';
      }
      var beforeLines;
      if (map) {
    ...
    if (!isProduction && !isTest && !isServer) {
      output += 'if (module.hot) {(function () {' + '  var hotAPI = require("' + hotReloadAPIPath + '")\n' + '  hotAPI.install(require("vue"), true)\n' + '  if (!hotAPI.compatible) return\n' + '  module.hot.accept()\n' + (
...
function processTemplate(part, filePath, parts) {
  if (!part) return _promise2.default.resolve();
...
function compileAsPromise(type, source, lang, filePath) {
  ...
  if (compile) {
    ...
    return _promise2.default.resolve(source);
  }
}

//# sourceMappingURL=compiler-compiled.js.map